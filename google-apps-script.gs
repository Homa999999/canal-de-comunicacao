...
function doPost(e) {
  try {
    ...
    if (descricao.length > 500) {
      return respostaJson({ sucesso: false, erro: "A descrição excede o limite de 500 caracteres." }, 400);
    }
...
function montarEmailHtml(dados) {
  ...
  const blocoAnexos = dados.anexos.length > 0
    ? [
      ...
      '<p style="margin:0 0 10px; font-size:12px; font-weight:700; color:#e6b800; text-transform:uppercase; letter-spacing:0.06em;">Anexos (' + dados.anexos.length + ")</p>",
